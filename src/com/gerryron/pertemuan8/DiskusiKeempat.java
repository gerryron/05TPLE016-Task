...
public class DiskusiKeempat {
	...
	private JComboBox jkCombo;
	private JComboBox jabatanCombo;
	private JComboBox lemburCombo;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					DiskusiKeempat window = new DiskusiKeempat();
	...
	private void initialize() {
		...
		frmDiskusiKeempat.setBounds(100, 100, 815, 559);
		...
		jabatanCombo = new JComboBox();
		jabatanCombo.addItem("- Pilih Jabatan");
		jabatanCombo.addItem("Operator");
		jabatanCombo.addItem("Cleaning Service");
		jabatanCombo.addItem("Satpam");
		jabatanCombo.addItem("Sopir");
		...
		jabatanCombo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (jabatanIndex == 1) {
					gapokField.setText("3500000");
				} else if (jabatanIndex == 2) {
					gapokField.setText("1750000");
				} else if (jabatanIndex == 3) {
					gapokField.setText("2000000");
				} else if (jabatanIndex == 4) {
					gapokField.setText("2500000");
				} else if (jabatanIndex == 5) {
					gapokField.setText("5000000");
				} else {
		...
		lemburCombo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (lemburComboIndex == 1) {
					upahLemburField.setText("100000");
				} else if (lemburComboIndex == 2) {
					upahLemburField.setText("200000");
			...
			public void actionPerformed(ActionEvent e) {	
				try {
					...
					if (!jamLemburField.equalsIgnoreCase("Tidak Lembur")) {
						jamLembur = new Short(Character.toString((jamLemburField.charAt(0))));
					}
					BigDecimal upahLembur = new BigDecimal(upahLemburField.getText());
		...
		btnHapus.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					int nikKaryawan = Integer.parseInt(nikField.getText());
					karyawanDao.deleteKaryawan(nikKaryawan);
					JOptionPane.showMessageDialog(null, "Success Delete Data Karyawan", "SUCCESS", JOptionPane.INFORMATION_MESSAGE);
				} catch(Exception ex) {
					JOptionPane.showMessageDialog(null, "Error Delete Data Karyawan : " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					JOptionPane.showMessageDialog(null, "Error Update Data Karyawan : " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
				}
	...
	public void showTable() {
		try {
			List<Karyawan> listKaryawan = karyawanDao.getAllKaryawan();
			tableModel = new KaryawanTableModel(listKaryawan);
			table.setModel(tableModel);
		} catch (Exception e) {
			JOptionPane.showMessageDialog(null, "Error : " + e, "Error", JOptionPane.ERROR_MESSAGE);
	...
	public void getComboBoxSelected(int index) {
		...
		if (jabatan.equals("Operator")) {
			jabatanCombo.setSelectedIndex(1);
		} else if (jabatan.equals("Cleaning Service")) {
			jabatanCombo.setSelectedIndex(2);
		} else if (jabatan.equals("Satpam")) {
			jabatanCombo.setSelectedIndex(3);
		} else if (jabatan.equals("Sopir")) {
			jabatanCombo.setSelectedIndex(4);
		} else if (jabatan.equals("Keuangan")) {
			jabatanCombo.setSelectedIndex(5);
		} else {
			jabatanCombo.setSelectedIndex(0);
		}
		
		if (jamLembur.equals("2")) {
